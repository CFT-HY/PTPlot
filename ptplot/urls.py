...
urlpatterns = [
    ...
    # Old name for models
    re_path(r'theories*',
            RedirectView.as_view(url='/ptplot/models', permanent=True)),
    

    # Old paths for plots didn't have the file extensions
    re_path(r'^(?P<anything>.*)ps$',
            RedirectView.as_view(url='/ptplot/%(anything)s''ps.svg',
                                 permanent=True,
                                 query_string=True)),
    re_path(r'^(?P<anything>.*)snr$',
            RedirectView.as_view(url='/ptplot/%(anything)s''snr.svg',
                                 permanent=True,
                                 query_string=True)),
    re_path(r'^(?P<anything>.*)snr_alphabeta$',
            RedirectView.as_view(url='/ptplot/%(anything)s''snr_alphabeta.svg',
                                 permanent=True,
                                 query_string=True)),
