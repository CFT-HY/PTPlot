...
urlpatterns: tp.List[URLPattern] = [
    ...
    path(r"snr_alphabeta.svg", views.snr_alphabeta_image, name="snr_alphabeta"),
    # SNR plot with UbarF on the x-axis and H_n R_* on the y-axis
    path(r"snr.svg", views.snr_image, name="snr"),
    ...
    # Display an individual model point on the SNR and PS plots
    path(r"models/<int:model_id>/<int:point_id>/snr.svg", views.model_point_snr, name="model_point_snr"),

    # Display an individual model point on the SNR and PS plots
    path(
        r"models/<int:model_id>/<int:point_id>/snr_alphabeta.svg",
    ...
    # Display a group of scenario points on the SNR plots
    path(
        r"models/<int:model_id>/scenarios/<int:scenario_id>/snr.svg",
        views.model_scenario_snr,
        name="model_scenario_snr"
    ),
    # Display a group of scenario points on the SNR plots
    path(
        r"models/<int:model_id>/scenarios/<int:scenario_id>/snr_alphabeta.svg",
        views.model_scenario_snr_alphabeta,
        name="model_scenario_snr_alphabeta"
    ),
    path(r"models/<int:model_id>/snr.svg", views.model_snr, name="model_snr"),
    path(r"models/<int:model_id>/snr_alphabeta.svg", views.model_snr_alphabeta, name="model_snr_alphabeta"),
    ...
    re_path(
        r'^(?P<anything>.*)snr$',
        RedirectView.as_view(
            url='/ptplot/%(anything)s''snr.svg',
            permanent=True,
            query_string=True
        )
    ),
    re_path(
        r'^(?P<anything>.*)snr_alphabeta$',
        RedirectView.as_view(
            url='/ptplot/%(anything)s''snr_alphabeta.svg',
            permanent=True,
            query_string=True
        )
    ),
